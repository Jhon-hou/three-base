...
import {MouseStyle,updateMouseStyle} from '../../other/mouseStyle'

var TrackballControls = function ( object, domElement,callback ) {
	...
	this.mouseButtons = { LEFT: MOUSE.DOLLY, MIDDLE: MOUSE.ZOOM, RIGHT: MOUSE.PAN };
	...
	this.update = function () {
		...
		callback&&callback(_this.target);
	...
	function mousedown( event ) {
		...
		if ( _state === STATE.NONE ) {
			...
			switch ( event.button ) {
				...
				case _this.mouseButtons.LEFT:
					updateMouseStyle(MouseStyle.mouseRotate);
					_state = STATE.ROTATE;
					break;
				...
				case _this.mouseButtons.RIGHT:
					updateMouseStyle(MouseStyle.mouseMove);
					_state = STATE.PAN;
					break;
				default:
					_state = STATE.NONE;
		...
		// updateMouseStyle(MouseStyle.default);
	...
	function mousewheel( event ) {
		...
		updateMouseStyle(MouseStyle.mouseManify);
		clearTimeout(_timer);
		_timer = setTimeout(()=>{
			updateMouseStyle();
		},250);
